...
for i in customers:
    if i!=j:
        prob += pulp.lpSum(x[(i, j, k)] for j in locations for k in range(len(trucks)))==pulp.lpSum(x[(j, i, k)] for j in locations for k in range(len(trucks)))==1, f"Flow_Balancing_{i}"
...
for k in range(len(trucks)):
    prob += pulp.lpSum(x[(depot1, j, k)] for j in customers) == 1, f"Leave_Depot_{k}"
...
for k in range(len(trucks)):
    prob += pulp.lpSum(x[(i, depot1, k)] for i in customers) == 1, f"Arrive_Depot_{k}"
